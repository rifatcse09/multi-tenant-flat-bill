...
<form method="POST" action="{{ route('owner.flats.update',$flat) }}"
  ...
  <label class="block mb-3">
    ...
    <input name="flat_number" value="{{ old('flat_number',$flat->flat_number) }}" class="mt-1 w-full border p-2 rounded" required>
  ...
  <div class="grid grid-cols-2 gap-3">
    <label class="block">
      ...
      <input name="flat_owner_name" value="{{ old('flat_owner_name',$flat->flat_owner_name) }}" class="mt-1 w-full border p-2 rounded">
      @error('flat_owner_name')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
    </label>
    <label class="block">
      <span class="text-sm text-gray-700">Phone</span>
      <input name="flat_owner_phone" value="{{ old('flat_owner_phone',$flat->flat_owner_phone) }}" class="mt-1 w-full border p-2 rounded">
  ...
  <div class="mt-5 flex gap-2">
    ...
    <a href="{{ route('owner.buildings.flats.index',$flat->building) }}" class="px-4 py-2 border rounded">Back</a>
  </div>
</form>
@endsection
